...
public class Fbo {
	 ...
	 */
	public Fbo(int width, int height, Integer numberOfsamples) {
		this.width = width;
		this.height = height;
		this.multiSampled = true;
		this.numberOfsamples = (int) numberOfsamples;
	...
	public void resolveTo(Fbo fbo) {
		...
		GL30.glBlitFramebuffer(0, 0, width, height, 0, 0, fbo.width, fbo.height, GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT,
			GL11.GL_NEAREST);
	...
	private void initialiseFrameBuffer(int type) {
		...
		if (multiSampled) {
			createMuitlsampleColorAttachment();
		} else {
	...
	private void createDepthTextureAttachment() {
		...
		GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL14.GL_DEPTH_COMPONENT24, width, height, 0, GL11.GL_DEPTH_COMPONENT, GL11.GL_FLOAT,
			(ByteBuffer) null);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER, GL11.GL_LINEAR);
		GL11.glTexParameteri(GL11.GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER, GL11.GL_LINEAR);
	...
	private void createMuitlsampleColorAttachment() {
		colorBuffer = GL30.glGenRenderbuffers();
		GL30.glBindRenderbuffer(GL30.GL_RENDERBUFFER, colorBuffer);
		GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, numberOfsamples, GL11.GL_RGBA8, width, height);
	...
	private void createDepthBufferAttachment() {
		...
		if (!multiSampled) {
			...
			GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, numberOfsamples, GL14.GL_DEPTH_COMPONENT24, width, height);
		}
